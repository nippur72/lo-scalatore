0 poke648,30:poke36869,240:poke36866,150:ox=0:print"{clr}":poke36869,255:poke36878,15:poke36879,25
...
10 gosub100:poke36869,255:e4=0:e5=2
15 gosub70:h=0:y=0
...
37 forn=1to5*5:next:pokes,t:pokes+g,d(t-j):s=s+22+di:t=peek(s):pokes,58:ifpeek(s+22)>61then55
38 pokes+g,0:poke36876,0:goto41
40 poke36876,200:poke36876,0:t=peek(s):pokes,58:pokes+g,0
41 ift=61thenss=ss+150:print"{home}{rvon}"tab(8-len(str$(ss)))ss:h=h+1:t=62:ifh=16then64
42 ift=60then55
43 gosub98
...
57 forn=1to17*5:next:so=so-5:ifso>150then56
58 poke36874,0:ch=ch-1:ifch=-1thenpoked,255:poke36869,240:print"{clr}{blk}"ss:end
...
64 ify=11then67
65 forn=y+1to11:poke(ox+7712)+b(n),62:ss=ss+100:print"{home}{rvon}"tab(8-len(str$(ss)))ss:gosub98
...
73 print"{rvon}"tab(8-len(str$(ss)))ss;tab(14)ch;tab(17)sc:poke(ox+7697),163:forn=0to11:poke(ox+7712)+b(n),60
